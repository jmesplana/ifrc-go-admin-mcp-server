...
function createEvent(event) {
  return chunkUILOG6SR_js.__spreadProps(chunkUILOG6SR_js.__spreadValues({}, event), {
    timestamp: Date.now()
  });
}

// src/lib/event-emitter.ts
var EventEmittingResponse = class extends http.ServerResponse {
  ...
  emitEvent(event) {
    if (this.onEvent) {
      this.onEvent(
        createEvent(chunkUILOG6SR_js.__spreadProps(chunkUILOG6SR_js.__spreadValues({}, event), {
...
var authContext = new async_hooks.AsyncLocalStorage();
...
function initializeRedis(_0) {
  return chunkUILOG6SR_js.__async(this, arguments, function* ({
    redisUrl,
    logger
  }) {
    ...
    redis = redis$1.createClient({
      url: redisUrl
    });
    redisPublisher = redis$1.createClient({
      url: redisUrl
    });
    redis.on("error", (err) => {
...
function initializeMcpApiHandler(initializeServer, serverOptions = {}, config = {
  ...
  } = _a, mcpServerOptions = chunkUILOG6SR_js.__objRest(_a, [
    "serverInfo"
  ]);
  ...
  let servers = [];
  let statelessServer;
  const statelessTransport = new streamableHttp_js.StreamableHTTPServerTransport({
    ...
    return chunkUILOG6SR_js.__async(this, null, function* () {
      ...
      if (url.pathname === streamableHttpEndpoint) {
        ...
        if (req.method === "POST") {
          ...
          if (!statelessServer) {
            statelessServer = new mcp_js.McpServer(serverInfo, mcpServerOptions);
        ...
        assert__default.default(sseMessageEndpoint, "sseMessageEndpoint is required");
        const transport = new sse_js.SSEServerTransport(sseMessageEndpoint, res);
        ...
        const server = new mcp_js.McpServer(serverInfo, serverOptions);
        ...
        let logs = [];
        const handleMessage = (message) => chunkUILOG6SR_js.__async(this, null, function* () {
        ...
        timeout = setTimeout(() => chunkUILOG6SR_js.__async(this, null, function* () {
          yield redis2.unsubscribe(`responses:${sessionId}:${requestId}`);
          sendResponse(408, "Request timed out");
        }), 10 * 1e3);
        res.on("close", () => chunkUILOG6SR_js.__async(this, null, function* () {
          hasResponded = true;
          clearTimeout(timeout);
          yield redis2.unsubscribe(`responses:${sessionId}:${requestId}`);
        }));
...
function withMcpAuth(handler, verifyToken, {
  ...
  return (req) => chunkUILOG6SR_js.__async(this, null, function* () {
    ...
    try {
      ...
      const publicError = new errors_js.InvalidTokenError("Invalid token");
...
function protectedResourceHandler({
  ...
  return (req) => {
    ...
    return new Response(JSON.stringify(metadata), {
      headers: chunkUILOG6SR_js.__spreadProps(chunkUILOG6SR_js.__spreadValues({}, corsHeaders), {
...
exports.createMcpHandler = createMcpRouteHandler;
exports.experimental_withMcpAuth = withMcpAuth;
exports.generateProtectedResourceMetadata = generateProtectedResourceMetadata;
exports.metadataCorsOptionsRequestHandler = metadataCorsOptionsRequestHandler;
